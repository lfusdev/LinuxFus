...
How to Use
Download the latest LinuxFus-v1.2.1-x86_64.AppImage from the Releases section.

Make it executable:
chmod +x LinuxFus-v1.2.1-x86_64.AppImage

Run with root privileges (needed for disk formatting and mounting):
sudo ./LinuxFus-v1.2.1-x86_64.AppImage

Select your USB drive and ISO file, then click START.

How It Works
Unmounts the target USB drive safely.

Prepares a FAT32 ESP partition alongside a main data partition using parted.

Copies the embedded C boot loader (/EFI/BOOT/BOOTX64.EFI) to the boot partition.

Mounts the source ISO and copies installation files to the target partition.

On boot, the loader executes the target OS's native boot file and steps out of the way.
